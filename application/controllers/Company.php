...
class Company extends Home_Controller {
    ...
    {   
        ...
        $data['menu'] = FALSE;
        $data['company'] = $this->common_model->get_by_slug($slug, 'business');
        //$data['service'] = $this->common_model->get_slug_by_company($service_slug, 'services', $data['company']->uid);
    ...
    public function get_time($date, $business_id)
    {
        ...
        $value = array();
        $value['times'] = get_time_by_days($day_id, $business_id);
        $value['date'] = $date;
        $data = array();
    ...
    public function check_coupon($code, $appointment_id)
    {
        ...
        if (empty($coupon)) {
            ...
            if (date('Y-m-d') >= $coupon->start_date && date('Y-m-d') <= $coupon->end_date) {
                ...
                echo json_encode(array('st' => 1, 'discount' => $coupon->discount, 'total_price' => $service->price, 'msg' => '<i class="fas fa-check-circle"></i> '.trans('coupon-applied-successfully')));
