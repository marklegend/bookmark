...
class Admin_model extends CI_Model {
    ...
    function get_appointments($user_id, $total, $limit, $offset)
    {
        ...
        $this->db->select('a.*, s.name as service_name, s.zoom_link, s.price, s.duration, f.name as staff_name, f.thumb as staff_thumb, c.name as customer_name, c.email as customer_email, c.thumb as customer_thumb, c.phone as customer_phone');
    ...
    function check_time($id, $date)
    {
        ...
        $this->db->where('service_id', $this->session->userdata('service_id'));
        $query = $this->db->get();
        $query = $query->row();
        if (isset($query)) {
            return true;
