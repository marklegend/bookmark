...
class Common_model extends CI_Model {
    ...
    function get_customer_appointments()
    {
        ...
        $this->db->select('a.*, s.name as service_name, s.zoom_link, s.price, s.duration, f.name as staff_name, c.name as customer_name, c.email as customer_email, c.phone as customer_phone');
    ...
    {
        $this->db->select('a.*, s.name as service_name, s.price, s.duration, f.name as staff_name, c.name as customer_name, c.email as customer_email, c.phone as customer_phone');
        $this->db->from('appointments a');
        $this->db->join('services s', 's.id = a.service_id', 'LEFT');
        $this->db->join('staffs f', 'f.id = a.staff_id', 'LEFT');
        $this->db->join('customers c', 'c.id = a.customer_id', 'LEFT');
